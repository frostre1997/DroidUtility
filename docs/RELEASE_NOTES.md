...
· Auto‑update checker – detects new releases from GitHub
...
· Auto‑versioning – version code resets to 1 on each new release tag
· GitHub Actions CI/CD – automatic builds and releases on push to main
· Cloudflare Pages hosting – clean, free, and unlimited bandwidth
· Auto‑update detection – checks GitHub API for latest release
...
· Status tab has been temporarily removed – will be re‑added in a future update with a more reliable implementation (using Build class instead of shell commands).
...
· Readd Status tab with crash free implementation
